...
export function PatientAppointmentsPage() {
  ...
  async function book(slot: Slot) {
    ...
    try {
      ...
      broadcastAppointmentsUpdated(); // ✅ update other tab
      alert("Booked!");
    } catch (err: any) {
  ...
  // ✅ listen cross-tab updates (doctor changes)
  useEffect(() => {
  ...
  return (
    <div className="stack">
              ...
              {loadingSlots ? (
                ...
                <div className="slot-wrap" style={{ marginTop: 8 }}>
                  {slots.slice(0, 30).map((s, idx) => (
                    <button key={idx} className="slot-pill" disabled={busy} onClick={() => book(s)} type="button">
                      {formatSlotLabel(s)}
                    </button>
                  ))}
                </div>
      ...
      {/* ✅ Patient modal: cancel + reschedule */}
      {selected && (
        <div
